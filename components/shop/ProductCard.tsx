...
function DiscountBadge({ pct }: { pct: number }) {
  ...
  return (
    <span className={cn("absolute left-3 top-3 z-10 rounded-full px-2.5 py-1 font-mono text-[11px] font-bold", tier)}>
      −{pct}%
    </span>
  );
}
...
export function ProductCard({ product, showVariants = false, requireAuth = false }: Props) {
  ...
  return (
    ...
    >
      ...
      {product.discount ? <DiscountBadge pct={product.discount} /> : null}
      ...
      <Link href={`/product/${product.id}`} aria-label={product.title} className="block">
        ...
        <div className="mt-1 flex items-center gap-2 text-xs text-fg-muted">
          ...
          {soldOut ? (
            <span className="font-mono text-[11px] text-danger">· out of stock</span>
          ) : remaining <= 5 ? (
            <span className="font-mono text-[11px] text-amber-400">· {remaining} left</span>
          ) : null}
        </div>
      ...
      <div className="mt-3 flex items-end justify-between">
        <div>
          ...
          {oldPrice && <p className="font-mono text-[11px] text-fg-muted line-through">{formatPrice(oldPrice)}</p>}
        </div>
        <motion.button
          type="button"
          onClick={handleAdd}
        ...
        >
          ...
          >
            {soldOut ? "Sold out" : inCartQty > 0 ? `Added ${inCartQty}` : "Add"}
          </span>
        </motion.button>
      </div>
    </motion.div>
